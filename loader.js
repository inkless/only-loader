...
;(function() {
  ...
  var addMultiEventListener = function(target, events, callback) {
  ...
  var stateChange = function() {
    ...
    while (pendingScripts[0] && (typeof pendingScripts[0] === 'function' ||
      pendingScripts[0].async ||
      pendingScripts[0].readyState == 'loaded' ||
      pendingScripts[0].readyState == 'complete')) {
  ...
  var loadExternalScript = function(src, async) {
    var load = function() {
      ...
      if (isModern) { // modern browsers
        ...
        script.async = async;
        script.src = src;
        addMultiEventListener(script, 'load error', removeFromPending.bind(undefined, script));
        head.appendChild(script);

        // only append to pendingScripts if its not async
        if (!async) {
  ...
  var loadInlineScript = function(func, async) {
    ...
    if (isModern) {
      for (var i = 0, len = pendingScripts.length; i < len; ++i) {
        ...
        ++pendingLoadedNum;
        addMultiEventListener(script, 'load error', onScriptLoaded);
      }
    }
