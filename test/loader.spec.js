...
describe('LoaderTest', function() {
  ...
  it('should execute function when load async script even it does not exist', function(done) {
    Loader('/not-exists.js', true);
    Loader(['/base/test/scripts/apple.js', function() {
      loaderConsole.log('inline');
    }]);
    Loader([function() {
      loaderConsole.log('inline2');
    ...
    setTimeout(function() {
      expect(loaderConsole.getLog()).toEqual(['apple', 'inline', 'inline2']);
      done();
    }, 300);
  });

  it('should execute function if script is not loaded after timeout', function(done) {
    Loader(['/not-exists.js', function() {
      loaderConsole.log('show');
    }]);
    setTimeout(function() {
      expect(loaderConsole.getLog()).toEqual(['show']);
      done();
    }, 300);
  });
